...
export function TacticResilienceSection({ start, end, tagIds }: Props) {
  const { data: tacticCoverage = [], isLoading } = api.analytics.coverage.byTactic.useQuery({ start, end, tagIds });
  ...
  const renderBar = (rate: number | null) => {
    if (rate === null)
      return <div className="h-2 bg-[var(--color-surface-elevated)] rounded-full" />;

    return (
      <div className="h-2 bg-[var(--color-surface-elevated)] rounded-full overflow-hidden flex">
        <div
  ...
  return (
    <Card ref={cardRef}>
      <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <ExportToPngButton targetRef={cardRef} fileName="scorecard-tactic-resilience" />
      </CardHeader>
      <CardContent>
            ...
            return (
              <Card key={tactic.tacticId}>
                <CardContent className="p-4">
                  <div className="mb-3">
                    <h4 className="font-medium text-[var(--color-text-primary)]">{tactic.tacticName}</h4>
                    <p className="text-xs text-[var(--color-text-muted)]">
                      Techniques: {tactic.plannedCount} planned, {executedSummary} across {tactic.operationCount} operations
                    </p>
                  </div>
                  <div className="space-y-3">
                    <div>
                      <div className="flex items-center justify-between text-xs mb-1">
                        ...
                        <span className={`font-medium ${getRateColor(tactic.detectionRate)}`}>
                          {formatRate(tactic.detectionRate)}{` (${tactic.detectionCount})`}
                        </span>
                      </div>
                    ...
                    <div>
                      <div className="flex items-center justify-between text-xs mb-1">
                        ...
                        <span className={`font-medium ${getRateColor(tactic.preventionRate)}`}>
                          {formatRate(tactic.preventionRate)}{` (${tactic.preventionCount})`}
                        </span>
                      </div>
                    ...
                    <div>
                      <div className="flex items-center justify-between text-xs mb-1">
                        ...
                        <span className={`font-medium ${getRateColor(tactic.attributionRate)}`}>
                          {formatRate(tactic.attributionRate)}{` (${tactic.attributionCount})`}
                        </span>
                      </div>
