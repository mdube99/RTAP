  ...
  effectiveness: viewerProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const toolEffectiveness = defensiveTools.map((tool) => {
        ...
        const detectionRate =
          detectionTotal > 0
            ? Math.round((detectionSuccess / detectionTotal) * 100)
            : 0;
        const preventionRate =
          preventionTotal > 0
            ? Math.round((preventionSuccess / preventionTotal) * 100)
            : 0;
        const attributionRate =
          attributionTotal > 0
            ? Math.round((attributionSuccess / attributionTotal) * 100)
            : 0;
      ...
      return {
        ...
        logSources: logSourceEffectiveness.sort(
