...
export async function getNextTechniqueSortOrder(
  db: PrismaClient,
  operationId: number,
): Promise<number> {
...
export function normalizeTechniqueTargetAssignments(
  assignments?: TechniqueTargetAssignment[],
): TechniqueTargetAssignment[] {
...
export async function ensureTargetsAssignableToOperation(
  ...
  const disallowed = assignments.filter(
    (assignment) => !allowedIds.has(assignment.targetId),
  );
  if (disallowed.length > 0) {
    throw new TRPCError({
      code: "BAD_REQUEST",
      message:
        "Targets must be associated to the operation before assigning them to a technique",
    });
  }
}

export async function createTechniqueWithValidations(
  db: PrismaClient,
  ...
  const normalizedAssignments = normalizeTechniqueTargetAssignments(
    input.targets,
  );
  ...
  if (input.mitreTechniqueId) {
    const mitreTechnique = await db.mitreTechnique.findUnique({
  ...
  if (input.mitreSubTechniqueId) {
    const mitreSubTechnique = await db.mitreSubTechnique.findUnique({
  ...
  return db.technique.create({
    data: {
      ...
      tools: input.toolIds
        ? { connect: input.toolIds.map((id) => ({ id })) }
        : undefined,
